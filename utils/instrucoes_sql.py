...
POPULAR_METADADOS = ("INSERT INTO dados(id_pagina, data_coleta, sha, texto_html) "
                     "VALUES ('{0}', #{1}#, '{2}', '{3}')")
...
POPULAR_DADOS = (
    ...
    "rptrObservacoes_ctl01_txtTexto, "
    "rptrAnotacoes_ctl01_txtTexto, "
...
SELECAO_IDS_BANCO_DE_DADOS = "SELECT [_dados_nao_tratados].[lblCodigo] FROM [_dados_nao_tratados];"

SELECAO_IDS_METADADOS = 'SELECT [dados].[id_pagina] FROM [dados];'

EXCLUSAO_DADOS = "DELETE FROM [_dados_nao_tratados] WHERE (([_dados_nao_tratados].[lblCodigo])='{}');"

EXCLUSAO_METADADOS = "DELETE FROM [dados] WHERE (([dados].[id_pagina])='{}');"
